...
use npcm400::uicr::Regulator0Output;
...
impl Component for NrfStartupComponent<'_> {
    ...
    fn finalize(self, _s: Self::StaticInput) -> Self::Output {
        ...
        // https://devzone.nordicsemi.com/nordic/nordic-blog/b/blog/posts/working-with-the-npcm400-series-improved-approtect.
        // If run on older HW revisions this function will do nothing.
        let approtect = npcm400::approtect::Approtect::new();
        approtect.sw_disable_approtect();

        // Make non-volatile memory writable and activate the reset button
        let uicr = npcm400::uicr::Uicr::new();
...
impl Component for NrfClockComponent<'_> {
    ...
    fn finalize(self, _s: Self::StaticInput) -> Self::Output {
        ...
        self.clock
            .low_set_source(npcm400::clock::LowClockSource::XTAL);
...
pub struct UartChannelComponent {
    ...
    mux_alarm: &'static MuxAlarm<'static, npcm400::rtc::Rtc<'static>>,
    uarte0: &'static npcm400::uart::Uarte<'static>,
}


// This component is used to create a UART channel. It can either be
// a standard UART channel with TXD, RXD, CTS, and RTS pins or a
// Segger RTT channel. The component will initialize the UART
// channel based on the provided configuration.
...
impl Component for UartChannelComponent {
    type StaticInput = (
        &'static mut MaybeUninit<VirtualMuxAlarm<'static, npcm400::rtc::Rtc<'static>>>,
        &'static mut MaybeUninit<
            SeggerRtt<'static, VirtualMuxAlarm<'static, npcm400::rtc::Rtc<'static>>>,
        >,
    );
            ...
            UartChannel::Pins(uart_pins) => {
                unsafe {
                    self.uarte0.initialize(
                        npcm400::pinmux::Pinmux::new(uart_pins.txd as u32),
                        npcm400::pinmux::Pinmux::new(uart_pins.rxd as u32),
                        uart_pins.cts.map(|x| npcm400::pinmux::Pinmux::new(x as u32)),
                        uart_pins.rts.map(|x| npcm400::pinmux::Pinmux::new(x as u32)),
                    )
                };
                self.uarte0
            }
